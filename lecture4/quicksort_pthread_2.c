...
void *qsort_pthread(void *ptr_arg)
{
	...
	int level = ((pthread_args_t*) ptr_arg)->level;
	...
	if (left < right)
	{
	    ...
	    if(level < MAX_LEVEL)
	    {
			r_pthread_args = (pthread_args_t){ a, swapIdx + 1, right, level + 1};
			l_pthread_args = (pthread_args_t){ a, left, swapIdx -1, level + 1};
	    ...
	    {
			r_pthread_args = (pthread_args_t){ a, swapIdx + 1, right, level};
			l_pthread_args = (pthread_args_t){ a, left, swapIdx -1, level};

	    	qsort_pthread(&l_pthread_args);
